 ...
 * http://dabeng.github.io/
 * 
 * Modified by Jordi Corbilla 2016
 * https://github.com/JordiCorbilla/OrgChart
 *
 * Licensed under the MIT license:
 * http://www.opensource.org/licenses/MIT
 */
  ...
  function createNode(nodeData, level, opts) {
    ...
    // construct the content of node
      var $nodeDiv = $('<div' +
          ...
          (nodeData[opts.nodeId] ? ' id="' + nodeData[opts.nodeId] + '"' : '') +
          '>');

      $nodeDiv.addClass('node ' + (nodeData.className || '') + (level >= opts.depth ? ' slide-up' : ''));
      if (opts.nodeContent !== 'undefined' && nodeData[opts.nodeContent] !== '') {
          if (nodeData["ismanager"] === "True")
              $nodeDiv.append('<div class="titleManager">' + nodeData[opts.nodeTitle] + '</div>');
          else
              $nodeDiv.append('<div class="title">' + nodeData[opts.nodeTitle] + '</div>');
      } else {
          $nodeDiv.append('<div class="titleDepartment">' + nodeData[opts.nodeTitle] + '</div>');
      }
      if (opts.nodeContent !== 'undefined' && nodeData[opts.nodeContent] !== '') {
          $nodeDiv.append(typeof opts.nodeContent !== 'undefined'
